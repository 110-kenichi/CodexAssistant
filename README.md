# Codex Assistant 0.3.0 — Visual Studio 2022
...
Proposed diffのApplyを実装しました。すべて保存してからApplyを押すと、Solution内のUTF-8テキストファイルを編集・追加・削除できます。既存ファイルのBOMとCRLF/LFを保持します。Rejectは未適用の提案を破棄します。

リポジトリ: [110-kenichi/CodexAssistant](https://github.com/110-kenichi/CodexAssistant)
...
既存版の更新は、Visual Studioを終了し、`CodexAssistant-0.3.0.vsix` を実行してください。拡張IDは0.1.0と同じです。
...
**Applyでソースへ反映できます。** CLIが読み取り専用で提案を生成し、拡張が一時フォルダーで差分を検証してから適用します。Sendだけではファイルを書き換えません。VS標準比較エディター・Undoとの連携は未実装です。
...
5. ApplyまたはGitコンテキストを使う場合はPATH上の `git.exe`。
...
6. 提案がある場合はProposed diffで内容を確認します。すべてのドキュメントを保存し、Applyでソースに適用します。外部変更の再読み込みをVisual Studioが求めた場合は再読み込みしてください。Rejectで未適用の提案を破棄できます。
...
PatchApplier.csが適用処理を担当します。Solution切替・未保存ドキュメント・差分の不一致・適用準備中の外部変更を検出して中止します。Solution外のパスやリンクは拒否します。UTF-8の通常テキストファイルのみ対応し、リネーム・バイナリ・UTF-16・混在改行・ファイル属性変更は対象外です。一度に100ファイル、元ファイル合計20MB、1ファイル10MBまで。書き込み失敗時は元の内容への復元を試みますが、ファイルロックなどで復元も失敗する場合があります。新規ファイルの旧形式プロジェクトへの自動登録とVSのUndo統合は未対応です。適用後の取り消しにはGit差分等を利用してください。

## 検証記録（2026-09-06）

0.3.0: C# / WPFコンパイル・VSIX生成成功。実ファイルの編集・追加・削除、UTF-8 BOM/CRLF保持、複数ファイルの競合時の変更防止、準備中の外部編集の保持、危険なパスの拒否、および既存の模擬CLI・モデル選択テストが成功しました。VS2022上のApply操作は実機未検証です。
